...
// 权重配置 - 数值越大影响力越大
const WEIGHTS = {
  basicInfo: 1.2, // 最重要 → 最大权重
  materializedPath: 1.0, // 次重要
  contentAnalysis: 0.8, // 第三重要
  tagKeywords: 0.6, // 最不重要但不至于太低
} as const;
...
console.log("3. 低权重单源: tagKeywords=0.8");
calculateMultiSourceScore({
  tagKeywords: 0.8,
});

console.log("4. 三源组合: basicInfo=0.7, materializedPath=0.8, contentAnalysis=0.6");
calculateMultiSourceScore({
  basicInfo: 0.7,
...
console.log("5. 全源命中: 0.6, 0.7, 0.8, 0.5");
calculateMultiSourceScore({
  basicInfo: 0.6,
...
console.log("6. 低confidence测试: basicInfo=0.3, contentAnalysis=0.4");
calculateMultiSourceScore({
  basicInfo: 0.3,
  contentAnalysis: 0.4,
});

console.log("7. 极端情况: basicInfo=1.0, contentAnalysis=1.0");
calculateMultiSourceScore({
  basicInfo: 1.0,
  contentAnalysis: 1.0,
});

console.log("8. 只有低权重源: materializedPath=0.9, tagKeywords=0.9");
calculateMultiSourceScore({
...
console.log("9. 都是0.7分数，不同source的权重影响:");
console.log("9a. basicInfo=0.7 (权重1.8，影响力最强)");
calculateMultiSourceScore({
  basicInfo: 0.7,
});

console.log("9b. materializedPath=0.7 (权重1.5)");
calculateMultiSourceScore({
  materializedPath: 0.7,
});

console.log("9c. contentAnalysis=0.7 (权重1.2)");
calculateMultiSourceScore({
  contentAnalysis: 0.7,
});

console.log("9d. tagKeywords=0.7 (权重0.6，影响力最弱)");
calculateMultiSourceScore({
  tagKeywords: 0.7,
});

console.log("10. 都是0.5分数，权重影响对比:");
console.log("10a. basicInfo=0.5");
calculateMultiSourceScore({
  basicInfo: 0.5,
});

console.log("10b. materializedPath=0.5");
calculateMultiSourceScore({
  materializedPath: 0.5,
});

console.log("10c. contentAnalysis=0.5");
calculateMultiSourceScore({
  contentAnalysis: 0.5,
});

console.log("10d. tagKeywords=0.5");
calculateMultiSourceScore({
  tagKeywords: 0.5,
});

console.log("11. 都是0.9分数，权重影响对比:");
console.log("11a. basicInfo=0.9");
calculateMultiSourceScore({
  basicInfo: 0.9,
});

console.log("11b. materializedPath=0.9");
calculateMultiSourceScore({
  materializedPath: 0.9,
});

console.log("11c. contentAnalysis=0.9");
calculateMultiSourceScore({
  contentAnalysis: 0.9,
});

console.log("11d. tagKeywords=0.9");
calculateMultiSourceScore({
  tagKeywords: 0.9,
});
...
function generateMarkdownTable(): string {
  let table =
    "| 测试用例 | basicInfo (1.2) | materializedPath (1.0) | contentAnalysis (0.8) | tagKeywords (0.6) | 总分 |\n";
  table +=
    "|----------|-----------------|------------------------|----------------------|------------------|------|\n";
