...
> {
	return withAuth(async ({ team: { id: teamId } }) => {
		try {
			// 获取基础统计
			const [totalCompleted, processing, pending, failed, totalAssets] =
				await Promise.all([
			...
			if (recentCompletedTasks.length > 0) {
				...
				avgProcessingTime = Math.round(
					totalProcessingTime / recentCompletedTasks.length / 1000,
				); // 转换为秒
			}
...
> {
	return withAuth(async ({ team: { id: teamId } }) => {
		try {
			...
			const total = Object.values(strategies).reduce(
				(sum, val) => sum + val,
				0,
			);
			...
			const data = [
				{
					...
					percentage:
						total > 0 ? Math.round((strategies.direct / total) * 100) : 0,
				},
				{
					name: "名称匹配",
					value: strategies.name,
					percentage:
						total > 0 ? Math.round((strategies.name / total) * 100) : 0,
				},
				{
					name: "内容匹配",
					value: strategies.content,
					percentage:
						total > 0 ? Math.round((strategies.content / total) * 100) : 0,
				},
				{
					name: "关键词匹配",
					value: strategies.keyword,
					percentage:
						total > 0 ? Math.round((strategies.keyword / total) * 100) : 0,
				},
			];
