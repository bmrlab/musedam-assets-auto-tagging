...
function getFallbackBox(meta: ProductImageMeta, t: TranslationFunction): ProductDetectionBox {
  return {
    xMin: 0,
    yMin: 0,
    xMax: meta.width,
...
function clampBox(box: ProductDetectionBox, meta: ProductImageMeta) {
...
function getDetectionLabelPosition(box: ProductDetectionBox, meta: ProductImageMeta) {
...
export default function ProductClassifyClient({ initialData }: { initialData: ProductLibraryPageData }) {
  ...
  async function handleClassify() {
    ...
    if (referenceProducts.length === 0) {
      toast.error(t("noReferenceProducts"));
      return;
    }
    ...
    <div className="flex min-h-[720px] flex-1 flex-col gap-6 px-1 py-5">
      <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
        ...
        <div className="rounded-[18px] border bg-background px-5 py-4 text-right">
          <div className="text-sm text-basic-5">{t("availableProducts")}</div>
          <div className="mt-1 text-3xl font-semibold text-basic-8">{referenceProducts.length}</div>
        ...
        <div className="space-y-6 rounded-[24px] border bg-background p-6">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
            ...
            <Button
              ...
              disabled={isRunning || !file || referenceProducts.length === 0}
            >
              {isRunning ? (
                <>
