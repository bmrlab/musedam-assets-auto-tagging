...
import { ProductItem, ProductTagTreeNode, ProductTypeItem } from "./types";
...
type ProductDialogProps = {
  ...
  usedProductTypeIds: string[];
  tags: ProductTagTreeNode[];
  onOpenChange: (open: boolean) => void;
  onSaved: (product: ProductItem) => void;
  onProductTypesChange: (types: ProductTypeItem[]) => void;
  onProductTypeRenamed: (typeId: string, name: string) => void;
  onProductTypeDeleted: (typeId: string) => void;
};
...
function buildDraftImages(product: ProductItem | null, t: TranslationFunction) {
  if (!product) {
    return [];
  }
  ...
  return product.images.map((image, index) => ({
    ...
    name: t("imageAltIndex", { name: product.name, index: index + 1 }),
...
export default function ProductDialog({
  ...
  useEffect(() => {
    ...
    setProductTypeId(product?.productTypeId ?? null);
    setDescription(product?.description ?? "");
    setSelectedTagIds(
      product?.tags.map((tag) => tag.assetTagId).filter((id): id is number => Boolean(id)) ?? [],
    );
    setNotes(product?.notes ?? "");
    setImages((current) => {
      revokeDraftImageUrls(current);
      return buildDraftImages(product, t);
    });
  }, [open, product, t]);
  ...
  const fallbackType =
    product?.productTypeId && !productTypes.some((type) => type.id === product.productTypeId)
      ? {
  ...
  const hasValidProductType =
    Boolean(productTypeId) && productTypes.some((type) => type.id === productTypeId);
  ...
  async function handleSelectImagesFromAssetLibrary() {
    try {
      ...
      const result = await prepareAssetLibraryProductImagesAction(
  ...
  function handleSubmit() {
    ...
    if (!productTypes.some((type) => type.id === productTypeId)) {
    ...
    startTransition(async () => {
      ...
      const result =
        mode === "create"
          ...
          : await updateAssetProductAction(formData);
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <div className="flex flex-col gap-2">
              <label className="h-[22px] text-[14px] leading-[22px] font-normal text-basic-8">
                {t("dialog.productNameLabel")}
              </label>
              <Input
                value={name}
                onChange={(event) => setName(event.target.value)}
                placeholder={t("dialog.productNamePlaceholder")}
            ...
            <div className="flex flex-col gap-2">
              ...
              <ProductTypeSelect
                ...
                onTypesChange={onProductTypesChange}
