...
export function getReviewablePersonRecommendationTagIds(
  personRecommendation: TaggingPersonRecommendation | null | undefined,
): number[] {
  if (!personRecommendation || !Array.isArray(personRecommendation.faces)) {
    return [];
  ...
  for (const face of personRecommendation.faces) {
    if (!isReviewablePersonFace(face) || !face.bestMatch) {
      continue;
    }
...
export function getReviewablePersonRecommendationTagIdsFromQueueResult(result: unknown): number[] {
  return getReviewablePersonRecommendationTagIds(getPersonRecommendationFromQueueResult(result));
}
