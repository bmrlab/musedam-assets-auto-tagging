...
- **独立关系**：多个候选描述的是彼此独立、可以同时成立的特征（例如投放渠道：抖音 / 天猫 / 小红书可以同时适用；风格标签也可能多个共存）。
...
- **最低门槛**: 只输出置信度≥0.4的预测（低于此值视为噪声，直接丢弃）；≥0.4 的候选请如实全部给出，不要因为识别模式是"精准"就自行只保留高分项——按识别模式做最终取舍是系统的工作，不是你的工作
